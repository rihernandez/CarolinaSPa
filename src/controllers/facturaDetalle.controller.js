...
exports.findOne = async(req, res) => {
    const facturaDetalle = await db.sequelize.query(
        "SELECT Facts.ID_Factura, FacD.ID_FacturaDetalle, FacD.CantidadProducto, Prod.nombreProducto, FacD.Precio, FacD.ITBIS, FacD.Total, Us.Nombre AS Us_Nomb, Us.Apellidos AS Us_Ap, CL.Nombre AS Cliente_Nomb, CL.Apellidos AS Cliente_Ap, EstF.Descripcion AS Estado_Fact, Facts.FechaFactura,Facts.FechaRegistro FROM facturaDetalles AS FacD inner join Facturas AS Facts ON FacD.ID_Factura = Facts.ID_Factura inner join Usuarios AS Us ON Facts.ID_Usuario = Us.ID_Usuario inner join Clientes AS CL ON Facts.ID_Cliente = CL.ID_Cliente inner join EstadoFacturas AS EstF ON Facts.ID_EstadoFactura = EstF.ID_EstadoFactura inner join Productos AS Prod ON Facts.ID_Producto = Prod.id_Producto WHERE FacD.ID_Factura = :ID_Factura", {
