...
class CreateProveedor extends Component {
    ...
    async componentDidMount() {
        ...
        if (this.props.match.params.id) {
            const res = await call("get", `proveedor/` + this.props.match.params.id);
    ...
    getProveedores = async() => {
        const res = await call("get", `proveedor`);
    ...
    onSubmit = async(e) => {
        ...
        if (this.state.editing) {
            await call("put", `proveedor/` + this.state.id_proveedor, {
                proveedor: this.state.proveedor,
            });

            // await axios.put(
            //     "http://localhost:8990/api/proveedor/" + this.state.id_proveedor, {
            //         proveedor: this.state.proveedor,
            //     }
            // );
            window.location.href = "/proveedor";
        } else {
            await call("post", `proveedor`, {
                proveedor: this.state.proveedor,
            });
    ...
    deleteProveedor = async(id) => {
        await call("delete", `proveedor/` + id);
